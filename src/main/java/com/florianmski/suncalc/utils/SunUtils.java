...
public class SunUtils
{
    ...
    {
        return Constants.TO_RAD * (357.5291 + 0.98560028 * d);
    }

    public static double getEquationOfCenter(double M)
    {
        return Constants.TO_RAD * (1.9148 * Math.sin(M) + 0.02 * Math.sin(2 * M) + 0.0003 * Math.sin(3 * M));
    }

    public static double getEclipticLongitude(double M, double C)
    {
        double P = Constants.TO_RAD * 102.9372; // perihelion of the Earth
